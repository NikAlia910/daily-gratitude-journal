...
/**
 * Performance test for the GratitudeEntry entity.
 *
...
public class GratitudeEntryGatlingTest extends Simulation {
    ...
    ChainBuilder scn = exec(http("First unauthenticated request").get("/api/account").headers(headersHttp).check(status().is(401)))
        ...
        .repeat(2)
        .on(
            exec(http("Get all gratitudeEntries").get("/api/gratitude-entries").headers(headersHttpAuthenticated).check(status().is(200)))
                .pause(Duration.ofSeconds(10), Duration.ofSeconds(20))
                .exec(
                    http("Create new gratitudeEntry")
                        ...
                        .asJson()
                        .check(status().is(201))
                        .check(headerRegex("Location", "(.*)").saveAs("new_gratitudeEntry_url"))
                )
                .exitHereIfFailed()
                .pause(10)
                .repeat(5)
                .on(exec(http("Get created gratitudeEntry").get("${new_gratitudeEntry_url}").headers(headersHttpAuthenticated)).pause(10))
                .exec(http("Delete created gratitudeEntry").delete("${new_gratitudeEntry_url}").headers(headersHttpAuthenticated))
                .pause(10)
        );

    ScenarioBuilder users = scenario("Test the GratitudeEntry entity").exec(scn);
